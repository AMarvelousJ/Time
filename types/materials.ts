...
export const MATERIALS = [
  ...
  { id: 20, name: '票决材料', stageId: 4, fields: ['voteTime'] },
...
export const TIME_RULES: Record<string, TimeRule> = {
  ...
  m17_branchPreReviewTime: { type: 'after', description: '综合政审后', dependencies: ['comprehensiveReviewTime'], config: { minDays: 0 } },
  ...
  probationPublicityTime: { type: 'after', description: '需在网络党校证明时间所在月份或之后', dependencies: ['onlineStudyTime'], config: { minDays: 0 } },

  // === 阶段 4: 预备党员阶段 ===
  // m19
  volunteerTime: { type: 'after', description: '一般早于支部大会1-2天', dependencies: ['probationPublicityTime'], config: { minDays: 0 } },
  branchMeetingTime: { type: 'after', description: '支部大会当天', dependencies: ['volunteerTime'], config: { minDays: 0 } },
  interviewTime: { type: 'range', description: '晚于支部大会，早于党委', dependencies: ['branchMeetingTime'], config: { minDays: 0 } },
  partyApprovalTime: { type: 'after', description: '党委会当天', dependencies: ['interviewTime'], config: { minDays: 0 } },
  m19_transferMeetingTime: { type: 'after', description: '预备期满（至少1年）', dependencies: ['branchMeetingTime', 'transferPublicityTime'], config: { minDays: 365 } },
  m19_transferApprovalTime: { type: 'after', description: '党委会当天', dependencies: ['partyApprovalTime'], config: { minDays: 0 } },
  
  voteTime: { type: 'sequential', description: '四张票决', dependencies: [], config: {} },
  ...
  probationObserveReportTime1: { type: 'quarterly', description: '1 季度 1 次', dependencies: ['probationObserveFormTime'], config: { minDays: 0, maxDays: 90 } },
  probationObserveReportTime2: { type: 'quarterly', description: '1 季度 1 次', dependencies: ['probationObserveReportTime1'], config: { minDays: 0, maxDays: 90 } },
  probationObserveReportTime3: { type: 'quarterly', description: '1 季度 1 次', dependencies: ['probationObserveReportTime2'], config: { minDays: 0, maxDays: 90 } },
  m22_branchSecretarySignTime: { type: 'after', description: '转正公示结束后', dependencies: ['probationObserveReportTime3'], config: { minDays: 0 } },
  m22_trainingTime: { type: 'sequential', description: '培训情况时间', dependencies: [], config: {} },
  m22_oathTime: { type: 'after', description: '党委审批通过预备党员后', dependencies: ['partyApprovalTime'], config: { minDays: 0 } },
  m22_contactPerson1Time: { type: 'after', description: '转正公示结束后', dependencies: ['m22_oathTime'], config: { minDays: 0 } },
  m22_contactPerson2Time: { type: 'after', description: '转正公示结束后', dependencies: ['m22_oathTime'], config: { minDays: 0 } },
  m22_partyGroupTime: { type: 'empty_field', description: '此栏空，不需要填', dependencies: [], config: {} },
  m22_committeeOpinionTime: { type: 'after', description: '转正公示结束后', dependencies: ['m22_oathTime'], config: { minDays: 0 } },
  ...
  // === 阶段 5: 转正阶段 ===
  transferApplicationTime: { type: 'range', description: '期满前半个月内提出', dependencies: ['branchMeetingTime'], config: { minDays: 335, maxDays: 395 } },
  transferPublicityTime: { type: 'after', description: '转正申请后，公示5个工作日', dependencies: ['transferApplicationTime'], config: { minDays: 0 } },
